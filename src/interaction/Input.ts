...
import { CONTENT, CURSOR } from './interactionEvent';
...
export default class Input {
  ...
  constructor(root: Root, dom: HTMLElement, listener: Listener) {
    ...
    const onCallback = (cb: () => void) => {
      if (this.node) {
        ...
        listener.emit([CONTENT, CURSOR], [this.node]);
      ...
      if (this.node) {
        if (keyCode === 13 || code === 'Enter') {
          ...
          if (!isIme) {
            ...
            listener.emit(CURSOR, [this.node]);
          }
        }
      }
    });
    ...
    this.onClick = (e) => {
      if (listener.state === State.EDIT_TEXT) {
        ...
        while (p) {
          if (this.ignoreBlurUnFocus.includes(p)) {
            return;
          }
  ...
  update(x: number, y: number) {
    const p = this.node!.setCursorStartByAbsCoords(x, y);
    this.updateCursor(p);
  }
  ...
  hide() {
    ...
    this.blur();
    if (this.node) {
      const empty = !this.node.content.length;
  ...
  updateCursor(p?: { x: number, y: number, h: number }) {
    ...
    if (p) {
      const canvas = this.node!.root!.canvas!;
      this.containerEl.style.left = p.x * canvas.clientWidth / canvas.width + 'px';
      this.containerEl.style.top = p.y * canvas.clientWidth / canvas.width + 'px';
      this.containerEl.style.height = p.h * canvas.clientWidth / canvas.width + 'px';
    }
  }
