...
class ResizeCommand extends AbstractCommand {
  ...
  execute() {
    ...
    nodes.forEach((node, i) => {
      ...
      if (clientRect) {
        ResizeCommand.operateMultiAr(node, computedStyle, cssStyle, dx, dy, controlType, clientRect, selectRect!, aspectRatio, fromCenter, widthToAuto, heightToAuto);
      }
      else {
        ResizeCommand.operate(node, computedStyle, cssStyle, dx, dy, controlType, aspectRatio, fromCenter, widthToAuto, heightToAuto);
      }
  ...
  undo() {
    const { nodes, data } = this;
    nodes.forEach((node, i) => {
      let { dx, dy, controlType, aspectRatio, clientRect, selectRect, fromCenter, widthFromAuto, heightFromAuto } = data[i];
      const originStyle = node.getStyle();
      node.startSizeChange();
      const computedStyle = node.getComputedStyle();
      const cssStyle = node.getCssStyle();
      controlType = flip(controlType);
      if (clientRect) {
        ResizeCommand.operateMultiAr(node, computedStyle, cssStyle, -dx, -dy, controlType, clientRect, selectRect!, aspectRatio, fromCenter, widthFromAuto, heightFromAuto);
      }
      else {
        ResizeCommand.operate(node, computedStyle, cssStyle, -dx, -dy, controlType, aspectRatio, fromCenter, widthFromAuto, heightFromAuto);
      }
      node.endSizeChange(originStyle);
      node.checkPosSizeUpward();
    });
  ...
  static operateMultiAr(node: Node, computedStyle: ComputedStyle, cssStyle: JStyle, dx: number, dy: number, controlType: CONTROL_TYPE,
    ...
    // 4个角在保持宽高比时需要计算对角线垂线交点，类似单个拖拽保持宽高比的计算
      ...
      if (aspectRatio) {
        const o = { width: selectRect.w, height: selectRect.h };
        const {
          x,
          y
