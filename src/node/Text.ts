...
class Text extends Node {
  ...
  override lay(data: LayoutData) {
    ...
    if (computedRich.length) {
      for (let i = 0, len = computedRich.length; i < len; i++) {
        const item = computedRich[i];
        SET_FONT_INDEX[item.location] = i;
        const family = item.fontFamily;
        font.waitIfNotRegister(family, () => {
    ...
    while (i < length) {
      ...
      // 每串富文本重置font测量
      if (i && setFontIndex) {
        const cur = computedRich[setFontIndex];
    ...
    // 水平非左对齐偏移
    if (computedRich && computedRich.length) {
      const hash: Record<number, ComputedRich> = {};
      computedRich.forEach(item => {
        hash[item.location] = item;
      });
      let textAlign = computedRich[0].textAlign;
  ...
  private expandRich(start: number, length: number) {
    const { rich, computedRich, style, _content: content } = this;
    // 空内容下，start会是0
    if (!rich.length) {
      const first = {
        location: start,
    ...
    for (let i = 0, len = rich.length; i < len; i++) {
      ...
      if (item.location <= start && item.location + item.length >= start) {
        if (start === item.location + item.length && isEnter(content.charAt(start - 1))) {
          continue;
        }
        item.length += length;
        computedRich[i].length += length;
        for (let j = i + 1; j < len; j++) {
          rich[j].location += length;
          computedRich[j].location += length;
        }
        return;
      }
    }
    ...
    computedRich[computedRich.length - 1].length += length;
  }

  private insertRich(modify: ModifyRichStyle, start: number, length: number) {
    ...
    const cst = this.calComputedRich(st);
    if (!rich.length) {
      rich.push(st);
      computedRich.push(cst);
      return;
    }
    for (let i = 0, len = rich.length; i < len; i++) {
      const item = rich[i];
      if (item.location <= start && item.location + item.length >= start) {
        ...
        if (item.location === start) {
          rich.splice(i, 0, st);
          computedRich.splice(i, 0, cst);
          item.location += length;
          computedRich[i].location += length;
        }
        else if (item.location + item.length === start) {
          rich.splice(i + 1, 0, st);
          computedRich.splice(i + 1, 0, cst);
        }
        else if (item.location + item.length > start) {
          ...
          const item2 = computedRich[i];
          const copy2 = clone(item2);
          item2.length = item.length;
          copy2.location = copy.location;
          copy2.length = copy.length;
          computedRich.splice(i + 1, 0, cst);
          computedRich.splice(i + 2, 0, copy2);
        }
        this.mergeRich();
        return;
      }
  ...
  private mergeRich() {
    let { rich, computedRich, _content: content, style } = this;
    if (!rich.length) {
      return;
    }
    ...
    if (content.length) {
      ...
      const last2 = computedRich[computedRich.length - 1];
      if (last.location + last.length > content.length) {
        last.length = content.length - last.location;
        last2.length = content.length - last2.location;
        hasMerge = true;
      }
      else if (last.location + last.length < content.length) {
        const o = {
          ...dft,
          location: last.location + last.length,
          length: content.length - last.location - last.length,
        };
        rich.push(o);
        computedRich.push(this.calComputedRich(o));
      }
      const first = rich[0];
      if (first.location > 0) {
        const o = {
        ...
        rich.unshift(o);
        computedRich.unshift(this.calComputedRich(o));
        hasMerge = true;
      }
    }
    for (let i = rich.length - 2; i >= 0; i--) {
      ...
      if (pn !== b.location) {
        if (pn < b.location) {
          ...
          rich.splice(i + 1, 0, o);
          computedRich.splice(i + 1, 0, this.calComputedRich(o));
        }
        else {
          const d = a.location + a.length - b.location;
          b.location += d;
          b.length -= d;
          computedRich[i + 1].location = b.location;
          computedRich[i + 1].length = b.length;
        }
        hasMerge = true;
        continue;
      }
      ...
      if (equal(a, b, [
        ...
        if (a.textAlign.v === b.textAlign.v || isEnter(content.charAt(b.location - 1))) {
          a.length += b.length;
          rich.splice(i + 1, 1);
          computedRich[i].length = a.length;
          computedRich.splice(i + 1, 1);
          hasMerge = true;
        }
      }
      // 如果\n单独形成样式，合并它（操作\n前后字符情况下会出现），因为它无意义
      else if (b.length === 1 && isEnter(content.charAt(b.location))) {
        a.length += b.length;
        rich.splice(i + 1, 1);
        computedRich[i].length = a.length;
        computedRich.splice(i + 1, 1);
  ...
  calComputedRich(rich: Rich) {
    return {
      ...
      fontStyle: rich.fontStyle.v,
      lineHeight: rich.lineHeight.v,
      letterSpacing: rich.letterSpacing.v,
      paragraphSpacing: rich.paragraphSpacing.v,
      textAlign: rich.textAlign.v,
      textDecoration: rich.textDecoration.map(item => item.v),
      color: rich.color.v,
      textShadow: rich.textShadow.v,
  ...
  override calReflowStyle() {
    super.calReflowStyle();
    this.computedRich = this.rich.map(item => {
      return this.calComputedRich(item);
    });
  }
  ...
  override calRepaintStyle(lv: RefreshLevel) {
    ...
    if (rich.length) {
      this.computedRich.forEach((item, i) => {
        ...
        item.color = rich[i].color.v;
        item.textShadow = rich[i].textShadow.v;
    ...
    if (isMulti && start !== end) {
      for (let i = 0, len = list.length; i < len; i++) {
        ...
        if (startLineBox === endLineBox) {
          ...
          let x1 = textBox.x + dx2;
          ctx.font = textBox.font;
          ctx.letterSpacing = textBox.letterSpacing + 'px';
          x1 += ctx.measureText(textBox.str.slice(0, startString)).width;
          textBox = list[endTextBox];
          let x2 = textBox.x + dx2;
          ctx.font = textBox.font;
          x2 += ctx.measureText(textBox.str.slice(0, endString)).width;
          ctx.fillRect(
            x1,
            lineBox.y + dy2,
            x2 - x1,
          ...
          if (textBox) {
            ...
            ctx.font = textBox.font;
            ctx.letterSpacing = textBox.letterSpacing + 'px';
            x1 += ctx.measureText(textBox.str.slice(0, endString)).width;
            ctx.fillRect(lineBox.x + dx2, lineBox.y + dy2, x1 - (lineBox.x + dx2), lineBox.lineHeight);
          }
        }
      }
    }
      ...
      for (let i = 0, len = lineBoxList.length; i < len; i++) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (textShadow) {
            ...
            ctx.shadowColor = color2rgbaStr(textShadow.color);
          }
          else {
            ctx.shadowOffsetX = 0;
            ctx.shadowOffsetY = 0;
          ...
          if (textDecoration.length) {
            textDecoration.forEach(item => {
              if (item === TEXT_DECORATION.UNDERLINE) {
                ...
                ctx.fillRect(
                  textBox.x + dx2,
                  (textBox.y + textBox.contentArea) + dy2 - h * 0.5,
                  textBox.w,
                  h,
                );
              }
                ...
                ctx.fillRect(
                  textBox.x + dx2,
                  (textBox.y + textBox.lineHeight * 0.5) + dy2 - h * 0.5,
                  textBox.w,
                  h,
                );
              }
  ...
  // 由于样式更新导致的lineBox重构，根据start/end将其它索引映射到重构后的正确索引
  reLocateCursor() {
    const { cursor, lineBoxList } = this;
    const { start, end, isMulti } = cursor;
    let count = 0;
    outer:
    for (let i = 0, len = lineBoxList.length; i < len; i++) {
      const lineBox = lineBoxList[i];
      for (let j = 0, len = lineBox.list.length; j < len; j++) {
        const textBox = lineBox.list[j];
        const n = count + textBox.str.length;
        if (n >= start) {
          cursor.startLineBox = i;
          cursor.startTextBox = j;
          cursor.startString = start - count;
          break outer;
        }
        count = n;
      }
    }
    if (!isMulti || end === start) {
      cursor.endLineBox = cursor.startLineBox;
      cursor.endTextBox = cursor.startTextBox;
      cursor.endString = cursor.startString;
    }
  ...
  moveCursor(code: number, isEnd = false) {
    ...
    this.inputStyle = undefined;
    const p = calPoint({ x: this.tempCursorX, y: lineBox.y }, matrix);
    return {
      x: p.x,
      y: p.y,
      h: lineBox.lineHeight * matrix[0],
  ...
  updateFormatRangeStyleData(location: number, length: number, modify: ModifyRichStyle) {
    ...
    if (rich.length) {
      for (let i = 0, len = rich.length; i < len; i++) {
        ...
        if (location >= item.location && location < item.location + item.length) {
          ...
          if (location > item.location) {
            ...
            computedRich.splice(i, 0, this.calComputedRich(prev));
          ...
          if (location + length < item.location + item.length) {
            ...
            computedRich.splice(i + 1, 0, this.calComputedRich(next));
            item2.length = item.length;
            shouldBreak = true;
          }
          // 可能存在的prev/next操作后（也可能没有），此Rich本身更新
          lv |= this.updateRichItem(item, item2, modify);
          if (shouldBreak) {
            break;
          }
      ...
      // 没有rich整体更新依旧不生成rich
      if (location === 0 && length >= this._content.length) {
        const r = this.updateFormatStyle(modify);
        if (r) {
          lv |= r;
        }
      }
        ...
        if (location > 0) {
          ...
          rich.push(dft);
          computedRich.push(this.calComputedRich(dft));
        }
        const o: any = {
          location,
          length,
        ...
        RICH_KEYS.forEach(k => {
          o[k] = style[k as keyof Style];
        });
        Object.assign(o, modify);
        rich.push(o);
        computedRich.push(this.calComputedRich(o));
        if (location + length < this._content.length) {
          const dft = normalizeRich({
            location: location + length,
            length: this._content.length - location - length,
          }, style);
          rich.push(dft);
          computedRich.push(this.calComputedRich(dft));
        }
  ...
  updateRichItem(item: Rich, c: ComputedRich, style: ModifyRichStyle) {
    ...
    if (style.fontSize && style.fontSize.v !== item.fontSize.v) {
      item.fontSize = style.fontSize;
      lv |= RefreshLevel.REFLOW;
    }
    if (style.color) {
      if (item.color.v[0] !== style.color.v[0]
        || item.color.v[1] !== style.color.v[1]
        || item.color.v[2] !== style.color.v[2]
        || item.color.v[3] !== style.color.v[3]) {
        item.color = style.color;
        lv |= RefreshLevel.REFLOW;
      }
    }
    if (style.letterSpacing && style.letterSpacing.v !== item.letterSpacing.v) {
      item.letterSpacing = style.letterSpacing;
      lv |= RefreshLevel.REFLOW;
    }
    if (style.lineHeight && style.lineHeight.v !== item.lineHeight.v) {
      item.lineHeight = style.lineHeight;
      lv |= RefreshLevel.REFLOW;
    }
    if (style.paragraphSpacing && style.paragraphSpacing.v !== item.paragraphSpacing.v) {
      item.paragraphSpacing = style.paragraphSpacing;
      lv |= RefreshLevel.REFLOW;
    }
    if (style.textAlign && style.textAlign.v !== item.textAlign.v) {
      item.textAlign = style.textAlign;
      lv |= RefreshLevel.REFLOW;
    }
    if (style.textDecoration) {
      if (style.textDecoration.length !== item.textDecoration.length) {
        item.textDecoration = style.textDecoration;
        lv |= RefreshLevel.REFLOW;
      }
      else {
        for (let i = 0, len = item.textDecoration.length; i < len; i++) {
          if (style.textDecoration[i].v !== item.textDecoration[i].v) {
            item.textDecoration = style.textDecoration;
            lv |= RefreshLevel.REFLOW;
            break;
          }
    ...
    if (style.textShadow && style.textShadow.v !== item.textShadow.v) {
